...
class ValikkoUI:
    """Luokka joka hallitsee valikon graafista kokonaisuutta"""

    def __init__(self, ikkuna: pygame.display, napit) -> None:
        self.ikkuna = ikkuna
        self.napit = napit
        self.luo_nappien_kuvat()
        # tämä atribuutti on sitä varten kun hiiri on napin päällä,
        # niin sille tulee oma väri
        self.aktiivisen_napin_tausta = pygame.Surface((0, 0))

    def luo_nappien_kuvat(self):
        """Luo napeille graafisen esityksen"""
        pygame.font.init()

        fontin_koko = konffi.leveys // 25
        for nappi in self.napit.sprites():
            fontti = pygame.font.SysFont(None, fontin_koko)
            teksti = fontti.render(nappi.napin_teksti, True, konffi.nappien_vari)
    ...
    def tee_hiiri_napin_paalla_effekti(self, nappi):
        """Luo 'hover' efektin napin päälle jossa hiiiri on, sekä vaihtaa kursoria"""
        if nappi:
    ...
    def luo_tekstit(
        self, leveys, korkeus, pelaaj1_teksti, pelaaja2_teksti, ruutujen_maara
    ):
        """Luo valikkoon tekstit (pois lukien napeissa olevat tekstit)"""
        ...
        i = 0
        for otsikko, teksti in teksti_dict.items():
    ...
    def piirra_valikko(
        ...
        """Piirtää valikon kaikki graagiset komponentit"""
